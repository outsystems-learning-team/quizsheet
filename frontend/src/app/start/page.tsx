...
const handleSelectQuestion = useCallback(async () => {
  setIsLoading(true);
  setError(null); // エラーをクリア

  try {
    // カテゴリを全選択
    const allCategoryNames = categories.map((c) => c.category_name);
    setSelectedCategories(allCategoryNames);

    const finalNumQuestions = selectedNumQuestionsOptionRef.current === "free"
      ? freeNumQuestionsRef.current
      : Number(selectedNumQuestionsOptionRef.current);

    // クイズデータを取得（questionOrder の指定はしない）
    const quizzesRes = await fetch(
      `/api/quizzes?quiz_name=${encodeURIComponent(activeQuizName)}&categories=${encodeURIComponent(allCategoryNames.join(','))}&limit=${finalNumQuestions}`
    );
    ...
    const finalQs: Question[] = questions.slice(
      0,
      finalNumQuestions
    );
